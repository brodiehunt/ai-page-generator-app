...
import { singleBlogSchema } from "@/src/schemas";
...
const initialBlogSections = [];

const initialOtherBlogSections = [
  "Introduction",
  "What is Term section",
  "How to Term Section",
  "History of term section",
...
const SpokeGeneratorClientWrapper = () => {
  const {
    control,
    register,
    handleSubmit,
  ...
  const [otherSections, setOtherSections] = useState(initialOtherBlogSections);
  const [buildSections, setBuildSections] = useState(initialBlogSections);
  ...
  const onSubmit = async (data) => {
    ...
    data.blogSections = buildSections;
    const stringData = JSON.stringify(data);
    try {
      const response = await fetch("/api/chatgpt/generateSingleSpoke", {
        method: "POST",
        body: stringData,
      });
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        {/* BLOG INFO SECTION */}
        <DashboardSectionHeader title="2. Blog Info" />
        <div className="grid grid-cols-2 gap-4 mb-4">
          <FormInput
            register={register}
            name="spokeTitle"
            type="text"
            label="Spoke Title"
            placeholder="Eg: Why small business can benefit from SEO"
            isPending={false}
            error={errors.spokeTitle}
          />
          <FormInput
            register={register}
            name="spokeSlug"
            ...
            placeholder="Eg: why-small-business-can-benefit-from-seo"
            isPending={false}
            error={errors.spokeSlug}
          />
          <FormInput
            register={register}
            name="spokeKeyphrase"
            type="text"
            label="Spoke Keyphrase"
            placeholder="Eg: Small business"
            isPending={false}
            error={errors.spokeKeyphrase}
          />
          <FormInput
            register={register}
            name="spokeTargetAudience"
            type="text"
            label="Spoke Target Audience"
            placeholder="Eg: Construction Business Owners"
            isPending={false}
            error={errors.spokeTargetAudience}
          />
          <div>
            <FormInput
              register={register}
        ...
        <DashboardSectionHeader title="3. Related Hub Info" />
        <div className="grid grid-cols-2 gap-4 mb-4">
          <FormInput
            register={register}
            name="hubName"
          ...
          <FormInput
            ...
            type="text"
            label="Hub Url"
            placeholder="Eg: https://cogdigital.com.au/search-engine-optimisation"
            isPending={false}
            error={errors.hubUrl}
          />
        </div>
        ...
        <DashboardSectionHeader title="4. Backlinks and integrated links" />
            ...
            {relatedSpokes &&
              relatedSpokes.map((item, index) => {
                return (
                  ...
                  //   <div
                  //     key={item.id}
                  //     className="mb-2 grid flex-grow grid-cols-2 gap-2"
                  //   >
                  //     <FormInput
                  //       type="text"
                  //       placeholder="Eg: Why spokes are so helpful "
        ...
        {/* BLOG Section Layout */}
        <DashboardSectionHeader title="5. Blog Section Layout" />
        {sectionsError && (
          <FormError
        ...
        <DashboardSectionHeader title="6. Import format" />
        ...
        <div className="mt-4">
          <FormButton isLoading={builderLoading}>Generate Blog</FormButton>
        </div>
      </form>
    </div>
  );
...
export default SpokeGeneratorClientWrapper;
